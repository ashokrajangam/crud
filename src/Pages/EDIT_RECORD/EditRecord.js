...
class EditRecord extends React.Component {
    constructor(props){
        ...
        this.edit_id = this.props.location.state.edit_id;
    }
    componentDidMount(){
        ...
        this.service.loadEditData(this.edit_id).then(response=>{    
            this.setState({...response.data[0], edit_id:this.edit_id});
        });
    ...
    handleSubmit = (e) => {
        ...
        if(this.validateForm()){
            this.service.updateRecord(this.state).then(response=>{
                if(response===1){
                    this.redirectToList();
                }
            });
